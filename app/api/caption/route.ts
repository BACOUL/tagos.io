...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) return bad("OPENAI_API_KEY manquante côté serveur", 500);
    if (!imageBase64) return bad("imageBase64 manquant");
    if (imageBase64.length > 7_000_000) return bad("Image trop lourde (> ~5 Mo)");

    const styleHint =
      style === "ecommerce"
        ? "Ton concis orienté produit, insiste sur matière/couleur/caractéristiques."
        : style === "editorial"
        ? "Ton descriptif naturel, ajoute un peu de contexte visuel."
